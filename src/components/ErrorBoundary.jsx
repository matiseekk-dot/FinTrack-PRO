    ...
    if (this.state.hasError) {
      return (
        <div style={{
          ...
          <h2 style={{ fontSize: 20, fontWeight: 700, marginBottom: 8 }}>{t("error.title", "Ojej, coś poszło nie tak")}</h2>
          <p style={{ fontSize: 14, color: "#64748b", marginBottom: 24, maxWidth: 340, lineHeight: 1.5 }}>
            {t("error.desc", "Apka napotkała błąd w tej sekcji. Twoje dane są bezpieczne — nie zostały utracone.")}
          </p>
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 10, width: "100%", maxWidth: 300 }}>
            ...
            <button onClick={() => window.location.reload()} style={{
              ...
              {t("error.reload", "Przeładuj apkę")}
            </button>
          </div>

          {import.meta.env.DEV && this.state.error && (
            <details style={{ marginTop: 24, fontSize: 11, color: "#475569", maxWidth: 500, textAlign: "left" }}>
              <summary style={{ cursor: "pointer", marginBottom: 8 }}>{t("error.details", "Szczegóły (dev)")}</summary>
              <pre style={{
